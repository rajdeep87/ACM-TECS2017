...
our tool \verifox.  We have experimentally compared \verifox with \hwcbmcv, 
which constructs a single monolithic SAT/SMT instance, and observe speedup of 
an order of magnitude over \hwcbmcv for proving safety properties as well as 
detecting firmware/hardware bugs.
%
\end{abstract}            
...
in UART IP. The FW CFG is shown on the left and the 
HW RTL code snippet is shown on the right. We abuse notations and 
...
in format that resembles SMT-LIB \sjcmt{which version? 1 or 2?} standard.  The bottom flow illustrates the
translation of C program into a static single assignment (SSA) form. 
...
$M$ for each unrolling \sjcmt{not always true}.
...
design intent of the interaction between a $\mathcal{ST}$\sjcmt{shouldn't it be FT?} and $\mathcal{HT}$, 
...
or a bug in the interface functions.  Hence, an important feature of a
...
SoC design.  All our experiments were performed on an Intel\textsuperscript{\textregistered} Xeon\textsuperscript{\textregistered} 3.07\,GHz
...
flow in \hwcbmcv. So, we only report bit-level results for \hwcbmcv in this paper. \sjcmt{I think this can backfire. The reivewers may ask why were other results not reported if available}\\ 
...
proving safety properties is on average $21\times$ \sjcmt{is it average for table 3 only? or also table 4?} for \verifox over
...
%
We obtained an open source System-on-a-Chip \sjcmt{or is it System-on-Chip?} design
...
using the Advanced Encryption Standard (AES) \sjcmt{cite if possible}.  There is a separate 
...
threads in an efficient manner.  Fortunately, a producer-consumer interaction pattern is
prevelant in many practical co-designs~\cite{polig2014micro,polig2014fpl,giefers2015accelerating,hvc}. 
...
embodied in \hwcbmcv and \verifox.  Our tools support co-designs written 
in Verilog RTL and C.  We also presented a specification language to 
effectively capture co-design properties.  To this end, we classify co-design 
properties into two types --  \emph{transaction level} and \emph{component level} 
property.  We successfully demonstrate the application of \hwcbmcv and \verifox 
for proving safety as well as finding critical co-design bugs for 
a UART IP and a complex SoC design.  

A FW exercises only a fragment of the HW state-space for a given scenario.  
...
bug finding.  In the future, we plan to extend our 
