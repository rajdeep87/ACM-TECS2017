...
of the $next\_timeframe()$ primitive in the interface functions.  Recall that a
call to $next\_timeframe()$ is used to advance the HW clock by one cycle. A call
to the software netlist emulates the same effect.  Furthermore, a software
netlist is a sequential model of the HW RTL represented in C.  Thus, the
