...
elements. Table~\ref{figure:transaction} shows few examples of transaction 
level property, written as monitors in C, for the UART IP.  In $monitor_P1()$, 
$outb(UART\_TR,0x0c)$ is a FW event and $ack\_o$ is a HW event. Whereas in
$monitor_P2$, the antecedent $tx\_empty()$ is a HW event and $send_data$ is a FW
event. 
%
\begin{figure}[htbp]
\scriptsize
...
\Omit{
The core supports single transmission/reception format which is
...
at the bit-level and word-level.  Column~7 gives the verification
times using \verifox.  Column~5~and~6 report
the total/feasible path counts and the percentage of infeasible paths pruned
for each mode.  The focus of our experiment is to compare the performance of 
path-based techniques over monolithic BMC. 
%effectively pruning infeasible FW-HW interactions with respect to a
%scenario.  is demonstrated by the \%-age of pruned paths (reported in
...
\hline
  & Bound & \multicolumn{2}{c|}{Monolithic} & \multicolumn{3}{c|}{Path-based} \\ 
\cline{3-7}
 Scenario &  & \multicolumn{2}{c|}{HW-CBMC} & \multicolumn{3}{c|}{VerifOx} \\ 
\cline{3-7}
      &       &  Bit-level & Word-level & Total/Feasible & \%-age & $\mathcal{M}_{seq}$  \\
%\cline{1-7}
      &       &   Time     &   Time      & Paths & Pruning & Time \\
\cline{1-7}      
\multicolumn{7}{|c|}{non-deterministic data but deterministic control (Scenario A)} \\ \hline
  transmit (32) & 250 & 15.02 & 15.78 & 247104/224 & 99.90 & \textbf{1}.{13} \\ 
transmit (64) & 500 & 23.87 & 23.29 & 247104/324 & 99.86 & \textbf{1}.\textbf{61}  \\ \hline 
\multicolumn{7}{|c|}{non-deterministic data and non-deterministic control
(Scenario B)} \\ \hline
  trans\_intr (32) & 250 & 14.86 & 15.98 & 247104/295 & 99.88 & \textbf{1}.{49} \\
  trans\_intr (64) & 500 & 24.14 & 26.42 & 247104/362 & 99.85 & \textbf{1}.{81} \\ \hline
\multicolumn{7}{|c|}{non-deterministic data and non-deterministic control
(scenario C)} \\ \hline
loopback (8)  & 230 & 52.06 & 55.71 & 247104/354 & 99.85 & \textbf{3}.\textbf{95} \\ 
loopback (16) & 500 & 122.12 & 147.65 & 247104/690 & 99.72 & \textbf{12}.\textbf{89} \\ 
loopback (32) & 650 & 170.62 & 192.35 & 247104/1282 & 99.48 & \textbf{21}.\textbf{85}   \\ 
loopback (64) & 1300 & 409.71 & 445.66 & 247104/2566 & 98.96 & \textbf{62}.\textbf{31}  \\ \hline
\multicolumn{7}{|c|}{detecting data-path bugs in transmission mode w/o interrupt} \\ \hline
  transmit (64) & 520 & 28.43 & 28.14 & 247104/324 & 99.86 &
  \textbf{1}.\textbf{12} \\ \hline
\multicolumn{7}{|c|}{detecting control bugs with interrupt enabled} \\ \hline
transmit (64) & 520 & 31.35 & 29.70 & 247104/362 & 99.85 & \textbf{1}.\textbf{05} \\ \hline
\multicolumn{7}{|c|}{detecting control bugs in loopback mode} \\ \hline
loopback (64) & 1300 & 443.15 & 427.68 & 247104/2566 & 98.96 & \textbf{62}.\textbf{34} \\ \hline
\end{tabular}
\end{scriptsize}
}
\end{center}
...
cryptographic accelerators.  The design statistics for SoC is given in

Table~\ref{table:stats}.  
The accelerator implements encryption/decryption using the Advanced Encryption
...
memory-mapped I/O interface.  The micro-controller communicates with the
accelerators and the XRAM by reading or writing to XRAM addresses.  The
arbitration of these module is done by the memory arbiter module.  

\Omit{
We synthesized the SoC to a software netlist (in C).  The generated model 
is bit-precise as well as cycle accurate.  The size of the
...
\hline
  & Bound & \multicolumn{2}{c|}{Monolithic} & \multicolumn{1}{c|}{Path-based} \\ 
\cline{3-5}
 Scenario &  & \multicolumn{2}{c|}{\hwcbmcv} & \verifox \\ 
\cline{3-5}
      &       &  Bit-level & Word-level & $\mathcal{M}_{seq}$ \\
\cline{1-5}
      &       &   Time     &   Time     & Time \\
\cline{1-5}      
\multicolumn{5}{|c|}{non-deterministic data and non-deterministic control (safe)} \\ \hline
data\_transfer & 20 & 86.92 & 86.23 & \textbf{17}.\textbf{42} \\ 
\hline
\multicolumn{5}{|c|}{non-deterministic data and non-deterministic control
(unsafe)} \\ \hline
data\_transfer & 20 & 92.63 & 91.28 & \textbf{1}.\textbf{27} \\ 
\hline
\end{tabular}
%\end{scriptsize}
}
