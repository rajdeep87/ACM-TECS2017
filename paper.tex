...
hardware/software co-design~\cite{dac17,taa,ibm-other}.  However, there is no 
automatic hardware/software co-verification tool that can exploit this interaction 
behavior.  For building effective and scalable formal co-verification tool, it is 
therefore imperative to automatically mine only relevant scenario/transaction pairs 
that occurs in practical co-designs.  In this paper, we attempt to bridge this gap by 
presenting a technique that automatically mine relevant HW/SW interaction behavior 
using symbolic execution.  A firmware services the requests from
the application software layer or its interacting hardware~\footnote{Note that whenever 
we refer to ``hardware'' from this point onwards, we mean an RTL implementation 
and not a TLM.}  components. 
