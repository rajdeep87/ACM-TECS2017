...
\begin{enumerate}
  ...
  \item We present two approaches for formal hardware/software co-verification
    ...
    pairs using symbolic execution.  To the best of our knowledge, this is the 
    first attempt to synthesize a pure software co-verification model from
    C/RTL co-designs specifically targeted for formal analysis.   
  ...
  \item We present an automatic HW/SW co-verifcation tool called, \hwcbmcv,
   ...
   \item We present an unified property specification language for
     co-verification that allows engineers to write meaningful 
...
respectively. \rmcmt{more on co-specification PSL}. 
\[
  for(i=0; i<=N; i++) 
...
simply denotes control-flow with no events.  The entires in the right column of
Table~\ref{transaction} shows the corresponding transaction in HW modules. We
simply denote a transaction in HW module with the name of the module,
...
\end{center}
\caption{\scriptsize Firmware Transactions and its interacting HW transaction}
\label{transaction}
\end{table}  
%
...
An unified co-verification model is constructed by instrumenting  
the software netlist model of HW RTL inside the FW thread, thereby 
generating a sequential single-threaded co-verification model in C.  
%
We now formally define a our co-verification model, denoted by 
$\mathcal{M}_{seq}$.\rmcmt{refer moduleName} as software netlist.
%
\[ 
\begin{array}[t]{@{}lll}
...
%===============================================================================
\section{Property Specification for Co-verification}
%===============================================================================
Lack of support for property specification in a HW/SW co-design is one of the
stumbling blocks for the application of formal techniques in co-verification.  
In this paper, we express a co-design property in C 
...
\end{center}
\caption{\scriptsize Design statistics for UART IP and SoC Design}
\label{table:stats}
\end{table}  
%
...
%
\textbf{Firmware Hardware Interaction Pattern in UART}
...
co-designs that exhibits producer-consumer relationship.  To this end, we construct 
a sequential co-verification model in \verifox through appropriate composition of 
HW and FW threads.  Our path-based algorithm in \verifox is able to suitably exploit 
