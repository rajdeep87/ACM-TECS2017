...
of the hardware RTL from the behavior expected by the firmware.  Formal
methods can deliver a proof that the RTL and the firmware behave correctly, but are
...
which constructs a single monolithic SAT/SMT instance, and observe an average 
speedup of $5\times$ over \hwcbmcv for proving safety properties as well as 
detecting critical co-design bugs.
%
\end{abstract}            
...
\scalebox{.55}{\import{figures/}{st.pspdftex}}
  \caption{Scenario (left) and Firmware Transactions (right)
\label{fig:st}}
\end{figure}
...
\textit{software netlist}~\cite{mtk2016} following synthesis 
...
models from RTL description, primarily targeted for formal 
verification~\cite{mtk2016,mskm2016,mkm2015}.  To the best of our knowledge, 
this is the first attempt to demonstrate the efficacy of such a 
translation in the context of hardware/software co-verification. 
% 
...
signals or local FW variables in $\mathcal{FT}$.  Arrows with no description
...
denote a transaction in HW with the name of the module, transmitter module 
in UART ($uart\_tx$), receiver module in UART ($uart\_rx$), or the top 
UART module ($uart\_top$).
%
\begin{table}
\begin{center}
{
...
the module hierarchy.  The interface signals $\{Var\_1,\dots,Var\_n\}$ 
...
in-tandem symbolic execution of HW and FW models.  
...
level of individual components of a co-design. Recall that a component 
...
\hwcbmcv is attributed to the path-based exploration of the 
...
pairs.  In the case of SoC scenario (data\_transfer), the FW exercises 
only the micro-controller and transfer sequence of bytes to the XRAM port 
...
co-designs that exhibits producer-consumer relationship.  Such interaction pattern 
is prevalent in many practical co-designs~\cite{polig2014micro,polig2014fpl,giefers2015accelerating,hvc}. 
