...
engineers write high-level software (SW) models for Intellectual Property (), 
...
Verilog/VHDL.   In this paper, we address some of these challenges to apply 
formal techniques in hardware/software co-verification.   
...
\hline
\end{tabular}
\caption{Firmware model of UART }
\label{firmware} 
\end{figure}
%
...
Here, $\mathcal{S}$ configures the UART  in {\em loopback} mode with 
interrupt enabled.
...
%\sjcmt{I think this can backfire. The reivewers may ask why were other results not reported if available}\\ 
...
proving safety properties is on average $21\times$ 
%\sjcmt{is it average for table 3 only? or also table 4?} 
for \verifox over \hwcbmcv.  The average speedup for detecting bugs is $48\times$ on average 
